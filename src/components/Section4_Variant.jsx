  ...
  return (
    <section>
      <div ref={textRef} className="animate-up">
        {/* SHOP NOW 텍스트 */}
        <p className={`shop-now-text text-reveal ${textInView ? 'in-view' : ''}`} style={{ transitionDelay: '0.2s' }}>
          {t('shop_now')}
        </p>
        
        {/* 메인 타이틀: 자연의 기적을 / 만나볼 차례 */}
        <h2 className={`text-reveal ${textInView ? 'in-view' : ''}`} style={{ transitionDelay: '0.4s' }}>
          <span style={{ display: 'block' }}>{t('product_title_1')}</span>
          <span style={{ display: 'block' }}>{t('product_title_2')}</span>
        </h2>
        ...
        {/* 제품명 */}
        <h3 className={`product-name text-reveal ${textInView ? 'in-view' : ''}`} style={{ transitionDelay: '0.6s' }}>
          {t('product_name')}
        </h3>
        
        {/* 용량 및 칼로리 */}
        <p className={`product-spec text-reveal ${textInView ? 'in-view' : ''}`} style={{ transitionDelay: '0.8s' }}>
          {t('product_spec')}
        </p>
        
        {/* 제품 설명 */}
        <p className={`product-desc text-reveal ${textInView ? 'in-view' : ''}`} style={{ transitionDelay: '1.0s' }}>
          {t('product_desc')}
        </p>

        {/* 구매하기 버튼 */}
        <a href="http://jjpyogo.com/goods/goods_view.php?goodsNo=1000000105" target="_blank" rel="noopener noreferrer" className={`text-reveal ${textInView ? 'in-view' : ''}`} style={{ transitionDelay: '1.2s', display: 'inline-block' }}>
